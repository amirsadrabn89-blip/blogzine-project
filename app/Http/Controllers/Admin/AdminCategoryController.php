...
class AdminCategoryController extends Controller
{
    public function index(Request $request): View
    {
        ...
        return view('pages.admin.categories_management.index', compact('categories', 'editingCategory'));
    }
    ...
    public function update(Request $request, Category $category): RedirectResponse
    {
        $validated = $request->validate([
            'title'       => ['required', 'string', 'max:50'],
            'slug'        => ['required', 'string', 'max:50', Rule::unique('categories', 'slug')->ignore($category->id)],
            'description' => ['nullable', 'string', 'max:200'],
            'is_show'     => ['nullable', 'boolean'],
        ], [
            'title.required'  => '● وارد کردن عنوان دسته‌بندی الزامی است.',
